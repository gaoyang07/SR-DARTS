...
def _ms_loop(dataset, index_queue, data_queue, done_event, collate_fn, scale, seed, init_fn, worker_id):
    ...
    try:
        ...
        watchdog = data.dataloader._utils.worker.ManagerWatchdog()

        while watchdog.is_alive():
            try:
                r = index_queue.get(timeout=data.dataloader._utils.MP_STATUS_CHECK_INTERVAL)
            except queue.Empty:
                continue
            ...
            try:
                ...
                data_queue.put((idx, data.dataloader._utils.ExceptionWrapper(sys.exc_info())))
            else:
        ...
        if self.num_workers > 0:
            ...
            if self.pin_memory:
                ...
                pin_memory_thread = threading.Thread(
                    target=data.dataloader._utils.pin_memory._pin_memory_loop,
                    args=(
                        self.worker_result_queue,
                        self.data_queue,
            ...
            data.dataloader._utils.signal_handling._set_SIGCHLD_handler()
